resource "random_password" "montandon_eoapi_db_admin" {
  length  = 16
  special = true
...
resource "random_password" "montandon_eoapi_db" {
  length  = 16
  special = true
...
resource "azurerm_postgresql_flexible_server" "montandon_eoapi" {
  name                   = "montandon_eoapi-${var.environment}-psql-flexible-server"
  ...
  administrator_password = random_password.montandon_eoapi_db_admin.result
...
resource "azurerm_postgresql_flexible_server_database" "montandon_eoapi" {
  name      = "montandon_eoapi-${var.environment}-db"
  server_id = azurerm_postgresql_flexible_server.montandon_eoapi.id
